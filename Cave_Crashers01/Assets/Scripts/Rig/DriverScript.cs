...
public class DriverScript : MonoBehaviour
{
    ...
    private void MountPlayer(GameObject player)
    {
        ...
        // Enable rigmovement
        RigMovement3 rigMovement = GetComponent<RigMovement3>();
        if (rigMovement != null)
        {
            if (rigMovement.enabled == false)
            {
                rigMovement.enabled = true;
            }
            rigMovement.SetPlayerInput(input);
        }
    ...
    {
        // Disable rig movement
        RigMovement3 rigMovement = GetComponent<RigMovement3>();
        if (rigMovement != null)
        {
            if (rigMovement.enabled == true)
            {
                rigMovement.enabled = false;
            }
            rigMovement.ClearPlayerInput();
        }
